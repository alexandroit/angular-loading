...
if (approved.size === 0) {
  console.log('Documentation dependency audit passed with no vulnerabilities.');
} else {
  console.log(
    'Documentation audit accepted only the unfixed image-size parser advisories ' +
      `through build tooling: ${[...approved].join(', ')}.`
  );
}
