...
function resolveTarget(target, doc = document) {
  if (typeof target === "string") {
    ...
    if (!found) {
      throw new Error(`@stackline/loading-core could not find target "${target}".`);
    }
    return found;
  }
...
var DOMLoader = class {
  constructor(options = {}) {
    ...
    if (!canUseDOM()) {
      throw new Error("@stackline/loading-core requires a browser-like DOM to create loaders.");
    }
  ...
  assertAlive() {
    if (this.state.destroyed) {
      throw new Error("@stackline/loading-core cannot operate on a destroyed loader.");
    }
  }
};
